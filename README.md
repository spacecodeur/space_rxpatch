...
```bash
git clone https://github.com/<your-username>/space_rxpatch.git
cd space_rxpatch
cargo build --release
```
...
`space_rxpatch <file_path> <regex_pattern> <replacement_text>`

### Examples

```
# Replace content inside curly braces
space_rxpatch example.txt '\{(.*?)\}' '{new content}'

# Insert a block between markers (multi-line safe)
space_rxpatch config.ini 'BEGIN\n(.*?)\nEND' 'BEGIN\ninserted line 1\ninserted line 2\nEND'
```

Note: escape characters like \n in CLI arguments may need to be passed carefully depending on your shell. A config file input is planned.

## Roadmap

- Support JSON/YAML-like structured inserts
- Add dry-run and diff preview
- CLI improvements with clap
- Cross-platform binary releases
- Unit tests for pattern types (inline, block, empty)
